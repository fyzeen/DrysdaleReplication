...
correlates_train = table2array(correlates(shuffled_indices(1:num_train), 1:13)); % SEE HOW WE ARE CUTTING AT 13!!
data_test = table2array(indexed_data(shuffled_indices(num_train+1:end), :));
correlates_test = table2array(correlates(shuffled_indices(num_train+1:end), 1:13));
...
[out] = rcc_matlab(table2array(indexed_data), table2array(correlates(:, 1:13)), gridsearch_out.lambda1, gridsearch_out.lambda1);
